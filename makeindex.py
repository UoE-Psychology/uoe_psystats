...
INCLUDED = ['dapr1','dapr2','dapr3','usmr','multivar','book','lectures']
...
ddesc={'': 'The Department of Psychology at the University of Edinburgh offers 3 Undergraduate Courses in statistics, and 2 postgraduate courses. ',
       ...
       'dapr1':'Data Analysis in Psychology using R-1 is a year-long course providing foundations in working with data and probability theory, and building to learning about how we make inferences about our hypotheses in psychology. We will deal with probability distributions, sampling and hypothesis testing, introducing simple statistical tests for two variables by way of example.<br><br>These concepts are demonstrated in R throughout, taking students from basic calculations to the foundations of data management, plotting and use of simple statistical tests.',
...
def fun(dir,rootdir):
    ...
    # this is for recursively indexing
    subdirnames = [fname for fname in sorted(os.listdir(dir)) if fname not in EXCLUDED]
    subdirnames = [fname for fname in subdirnames if fname not in filenames]
    subdirnames = [fname for fname in subdirnames if fname not in EXCLUDEDDIR]
    
    # this is for the lists
    dirnames = [fname for fname in sorted(os.listdir(dir)) if fname in INCLUDED]
