...
```{r tpb, fig.cap="Theory of planned behaviour (latent variables only)"}
knitr::include_graphics("images/TPB example.png")
```

The data is available either:
+ as an .RData file here: []()
+ as a .txt file here: []() 
...
`r msmbstyle::question_begin(header="&#x25BA; Question 0")`
...
Read in the data using the appropriate function.
`r msmbstyle::question_end()` 
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS)`
```{r read in data}
TPB_data<-read.csv("data/TPB_data.csv")
```
