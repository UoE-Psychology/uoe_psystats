...
+ The lecture slides can be accessed [here](https://uoe-psychology.github.io/uoe_psystats/multivar/lectures/week7_pca.pdf).   
...
Begin by reading in the new data, available at [url](url) 

`r msmbstyle::question_end()` 
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS)`

```{r}
df <- read.csv("data/conduct_problems_2.csv")
```
...
*Remember:* If $\textrm{RMSEA} < .05$, $\textrm{SRMR} < .05$, $\texrm{TLI} > .95$ and $\textrm{CFI} > .95$ then the model fits well. Smaller values of RMSEA and SRMR mean better fit while larger values of CFI and TLI mean better fit. 
`r msmbstyle::question_end()` 
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS)`
...
Write a short paragraph summarising the method and results of the two factor model. You do not need to include a table or figure with parameter estimates.
`r msmbstyle::question_end()` 
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS)`
