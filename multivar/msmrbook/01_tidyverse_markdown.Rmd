...
`r task()`If you haven't previously installed them, install the following packages:  
...
`r task()`Create a new `R Notebook` file, fill it in with the content of your script from the exercises so far.   
...
`r task()` Knit the notebook into a HTML file.  
