...
Read in the data and it a one-factor confirmatory factor analysis using an appropraite estimation method.
`r msmbstyle::question_end()` 
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS)`
```{r categorical estimation}
stress_IQ_data<-read_csv("data/Stress_IQ_data.csv")
...
model_stress.est<-cfa(model_stress, data=stress_IQ_data, ordered=c('stress1','stress2','stress3','stress4','stress5'))
...
plot(d_IQ1, main='', xlab='', ylab=' ') # plots the results
d_IQ2<- density(stress_IQ_data$IQ2) # returns the density data
plot(d_IQ2, main='', xlab='', ylab=' ') # plots the results
d_IQ3<- density(stress_IQ_data$IQ3) # returns the density data
plot(d_IQ3, main='', xlab='', ylab=' ') # plots the results
d_IQ4 <- density(stress_IQ_data$IQ4) # returns the density data
plot(d_IQ4, main='', xlab='', ylab=' ') # plots the results
d_IQ5 <- density(stress_IQ_data$IQ5) # returns the density data
plot(d_IQ5, main='', xlab='', ylab=' ') # plots the results
```
...
SEM_model.est<-sem(SEM_model, data=stress_IQ_data, ordered=c('stress1','stress2','stress3','stress4','stress5'))
...
In order to try and replicate the IQ CFA, our researcher collects a new sample of n=500. However, she has some missing data. Specifically, those who scored poorly on earlier tests tended to feel discouraged and chose not to complete further tests.  
  
Conduct a CFA using the new data, taking account of the missingness.  
  
The data can be found at [URL](), and is in .csv format. 
`r msmbstyle::question_end()` 
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS)`
...
```{r missingness}
IQ_data_new<-read_csv('data/new_IQ_data.csv')

IQ_model_missing<-'IQ=~IQ1+IQ2+IQ3+IQ4+IQ5
IQ1~~IQ2
IQ4~~IQ5'

IQ_model_missing.est<-cfa(IQ_model_missing, data=IQ_data_new, missing='FIML')
