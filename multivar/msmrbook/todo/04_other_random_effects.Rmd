...
## Exercise 3 - Handling singular fits.  
...
This warning us that our model has resulted in a 'singular fit'. You can read in depth about what this means by reading the help documentation for `?isSingular`. For our purposes, a relevant section is copied below: 
```
...
What we're doing here is following Barr et al.'s advice of defining our maximal model and then removing only the terms to allow a non-singular fit.  
`r msmbstyle::question_end()` 
`r msmbstyle::solution_begin(hidden=FALSE)`
```{r}
VarCorr(m1)
