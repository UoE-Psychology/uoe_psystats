...
- The Rmarkdown file for this week is TODO URL.
...
Because of this, any conclusion we might make from the sample is not generalizable from the whole population.
...
Any sample selection method that is biased, i.e. leading to samples which are not representative of the entire population, will mean that the results we obtain from the data in the sample can not be generalized to the entire population._
...
Since ACME Corporation has such a big mail order catalogue, see Figure \@ref(fig:acme), we will assume that the company sells many products. Furthermore, we only have the catalogue in paper-form and no website or online list of prices is available.
...
- The greek letter $\mu$ (mu) is used as a parameter to denote the population mean/average, while $\bar{x}$ (x-bar) or $\hat{\mu}$ (mu-hat) as a statistic for the mean computed on a sample.
...
We can plot the two means using a histogram, adding a vertical red line denoting the true population mean:
...
By taking repeated samples of size equal to the entire population, every time we would obtain the population parameter exactly, so the distribution would look like a histogram with a single bar on top of the true value: we would find the true parameter with probability one.
`r msmbstyle::solution_end()`
...
```{r, echo=FALSE, out.width='65%', fig.align='center', fig.cap='Comparing accuracy and precision. (From www.moderndive.com)'}
...
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS, toggle=TOGGLE)`
```{r, message=FALSE, fig.cap="Sampling distribution of the proportion for $n = 20$ with population parameter $p$ marked by a red vertical line."}
...
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS, toggle=TOGGLE)`
```{r, message=FALSE, fig.height=8, fig.width=7, fig.align='center', fig.cap="Three sampling distributions of the proportion, with population parameter $p$ marked by a red vertical line."}
