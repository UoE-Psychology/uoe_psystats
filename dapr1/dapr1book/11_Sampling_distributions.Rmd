...
**Selecting variables and dropping NA (Not Available) values**

- Extract from the `hollywood` tibble the three variables of interest (Movie, Genre, Budget).
- Keep only the movies for which we have all information, i.e. no entries which are NA.

`r msmbstyle::question_end()`

`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS, toggle=TOGGLE)`
To keep only some of the variables in a tibble, we use the function `select()`. The following code keeps only the variables Movie, Genre and Budget:
```{r}
hollywood <- hollywood %>%
  select(Movie, Genre, Budget)
```

If you look through the `hollywood` tibble by running the command `View(hollywood)` in the R Console, you will see that data on the Genre or Budget is **Not Available (NA)** for some movies.
In order to only keep the movies for which we have full information, i.e. no NA values, we can either use the function `drop_na()` or `na.omit()`. You only need to use one of the two, pick your favourite!

**Option 1: Using `drop_na`.**
```{r}
# Option 1: Using drop_na instead of na.omit
hollywood <- hollywood %>%
  drop_na()
```

Now you have a tibble without missing values.

Look at the help of the function for more information:
```{r}
?drop_na
```
...
You would have also reached the same conclusion by replacing `drop_na()` with `na.omit()`:
```{r eval=FALSE}
# Option 2: Using na.omit instead of drop_na
hollywood <- hollywood %>%
  na.omit()
```
...
Note that you only have to run one of `drop_na` or `na.omit`, no need to run both.
`r msmbstyle::solution_end()`
...
`r msmbstyle::solution_begin(hidden=HIDDEN_SOLS, toggle=TOGGLE)`
Remember that a proportion is calculated as a count divided by the total. The count of Comedy movies is found by finding first which movies have `Genre == "Comedy"`. Note the double equal `==` which **tests for equality**. Then we count the total number of Comedy movies by summing the values which are TRUE, `sum(Genre == "Comedy")` and dividing by the total `n()`. Note that `n()` is a function of tidyverse here. However, since the mean is also the sum divided by n, we can just use the function `mean`:
...
`r msmbstyle::question_begin(header = "&#x25BA; Question 8 (Optional)")`
