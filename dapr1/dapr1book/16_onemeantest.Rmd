...
Over the last 5 weeks, we have started to look at how we can use a sample from a population to draw infereces about the population.  
<br>  
...
+ On the Beck Depression Inventory (BDI), a score of >25 is considered clinical diagnosis of depression. Is the average score of people with --- significantly above this cutoff?  __TODO: unclear__

### Hypotheses  

**Null hypothesis:**  

+ The population mean ($\mu$) is equal to some pre-specified number (denoted $\mu_{0}$).  
  + $H_0: \mu = \mu_{0}$  

**Alternative hypothesis:** 

+ The population mean ($\mu$) is not equal to/is less than/is greater than some pre-specified number (denoted $\mu_{0}$).  
  + $H_1: \mu \neq \mu_{0}$  
  + $H_1: \mu > \mu_{0}$  
  + $H_1: \mu < \mu_{0}$  
...
Recall that in [Week 13](#chap-hyp-test), we introduced the concept of a **standardized statistic**, typically denoted by $z$.
...
$$
z = \frac{\textrm{statistic} - \textrm{mean of null distribution}}{\textrm{standard deviation of null distribution}} = \frac{\bar{x} - \mu_{0}}{\sigma/\sqrt{n}}
$$
...
If we perform a one-tailed test of $\mu < \mu_{0}$ on a sample of 6, we will reject the null hypothesis ($\mu = \mu_{0}$) if our corresponding $t$-statistic is $\leq -2.015$.  
...
```
If we perform a *two*-tailed test of $\mu \neq \mu_{0}$ on a sample of 6, we will reject the null hypothesis ($\mu = \mu_{0}$) if the **absolute magnitude** of our corresponding $t$-statistic is $\geq 2.571$.  
...
__TODO: isn't clear - requires students to clearly see that the sample is already from the rock category. Perhaps add a note?__
...
$$
t \qquad = \qquad \frac{? \qquad - \qquad ?}{?}
$$
`r msmbstyle::question_end()`
`r msmbstyle::solution_begin(hidden=FALSE)`
Fill in the blanks:  
...
and write out the $t$-statistic: 
$$
t  =  \frac{128.9 - 125}{26.5 / \sqrt{200}}
$$
`r msmbstyle::solution_end()`

We can do this quickly using `R` just like a calculator:
```{r}
t_stat = (128.9 - 125) / (26.5 / sqrt(200))
t_stat
```
...
__TODO: can be confusing for students, I'd just introduce the relevant quantile function, if the alternative is greater than, I'd use the upper tail only.__
```{r}
...
One last important thing to note is that when we perform one sample mean tests, we assume a few basic things. These assumptions need to be satisfied in order for the inference procedure to be valid:

1. The data are continuous (not discrete);
2. The data are normally distributed OR the sample size is large enough (rule-of-thumb $n$ = 20) and the data are not strongly skewed;
...
+ When we do not know the population standard deviation ($\sigma$), we compute the standardised statistic $t$ using the formula $t = \frac{\bar{x} - \mu_{0}}{s/\sqrt{n}}$.  
