...
**LO1.** Understand the null and alternative hypotheses and how to specify them for a given research question.

**LO2.** Understand statistical significance and how to calculate p-values from null distributions.

**LO3.** Understand how to shift a bootstrap distribution to obtain a null distribution centred at the parameter specified in the null hypothesis.
...
## Recap
In [Week 11](#chap-sampling-distributions) we saw that a population is described by parameters and samples by statistics. We typically can not measure the entire population, so parameters need to be estimated by sample statistics. We quantified the uncertainty in our estimate by standard error (SE) of the sample statistic. This is defined as the standard deviation of the sampling distribution, showing the sample statistic computed on many samples from the population.
In practice, however, we are not able to quantify the SE by taking repeated samples from the population, and we have only one sample of size $n$. In [Week 12](#bootstrapping-and-confidence-intervals) we learned to approximate the sampling distribution by bootstrapping.
...
Using the centre of the bootstrap distribution, and the estimated SE, we learned how to give a range of plausible values for the population parameter.
...
- *Significance level.* A threshold used as a criterion for deciding when a p-value is small enough to provide convincing evidence against the null hypothesis.
