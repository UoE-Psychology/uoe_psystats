...
| P-value | $$ \begin{array}{ll}   i.   & \Pr(T_{df} \leq t) \\   ii.  & \Pr(T_{df} \geq t) \\   iii. & 2 \times \Pr(T_{df} \geq |t|) \end{array} $$ | $$ \begin{array}{ll}   i.   & \Pr(T_{df} \leq t) \\   ii.  & \Pr(T_{df} \geq t) \\   iii. & 2 \times \Pr(T_{df} \geq |t|) \end{array} $$ | $$ \begin{array}{ll}   i.   & \Pr(T_{df} \leq t) \\   ii.  & \Pr(T_{df} \geq t) \\   iii. & 2 \times \Pr(T_{df} \geq |t|) \end{array}  $$ |
...
The data are stored in the file [`writers.txt`](https://edin.ac/2Ul2yLj).
...
Note the use of `rep()` here.  
`rep(x, times)` will repeat `x` a given number of `times`. For instance:
```{r}
rep("a", 4)
```

In the code chunk above where we assign `writers_table`, we use the `rep()` function to take the value we assigned as `n` (i.e., `nrow(writers)`), multiplied by $1/7$, and then repeat that value 7 times. 
`r msmbstyle::solution_end()`
...
The data are stored in the file [`TipJoke.csv`](https://edin.ac/2U6zfgO). 
The variables are:
...
- Response variable: Whether or not the customer left a tip.
...
- _Cell contribution._ The contribution of a cell in a two-way table to the chi-square statistic. It is helpful in determining where large differences between observed data and what would be expected if the null hypothesis were true, exist.
...
- _Expected counts._ The expected frequencies for the cells of the two-way table assuming the null hypothesis is true; found by applying the row proportions to the column totals.
- _Observed counts._ The observed frequencies in the cells of the two-way table from the study.
- _Validity conditions for chi-square test._ Each cell of the two-way table must have at least 5 observations.
