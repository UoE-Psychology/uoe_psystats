...
The entities about which data are collected are called __observational units__.

The number of observational units in the study is known as the __sample size__, denoted by $n$.
...
The observational units are the sampled adult Americans participating in the 2004 General Social Survey (GSS).
...
`r msmbstyle::solution_begin()`
The recorded variables are the sex of the participant and the number of names given by the participant. The former is categorical and binary, while the latter is a numerical value that varies from participant to participant. We are interested in how the number of names tends to vary with the sex of the participant. For this reason, sex is the explanatory variable, while number of close friends in the response variable.

| Variable's name         | Type                        | Role             |
|:------------------------|:---------------------------:|:----------------:|
| Sex                     | Categorical and binary [^1] | Explanatory      |
| Number of close friends | Quantitative                | Response         |

[^1]: **Note**: This survey was conducted in the US in the year 2004. We are merely analysing the data that were collected in that survey as it is an extensive and interesting dataset.
The fact that we are presenting this study as an in-class example is not an endorsement to the view that gender is a binary variable. 
**In that specific survey**, the recorded values of the variable Sex were `female` and `male`, and we are merely analysing the available open-source data collected by the US officials.
