...
`r msmbstyle::solution_begin(header = "&#x25BA; Example", hidden = FALSE, toggle = TOGGLE)`
Do peoples' scores differ between timepoint one and timepoint two?  

+ Same participants and same test administered at multiple timepoints   
...
`r msmbstyle::solution_begin(header = "&#x25BA; Example", hidden = FALSE, toggle = TOGGLE)` 
Do peoples' scores on maths tests differ from their scores on english tests?  

+ Same people taking both tests and producing a score on maths and score on english   
...
+ Same variable (hours worked per day) collected from each person in pairs of people (i.e., each employee and their respective bosses).  
...
<center>
$\mathrm{Pr}(T_{df} \leq t)$
$\mathrm{Pr}(T_{df} \geq t)$
$2 \times \mathrm{Pr}(T_{df} \leq |t|)$
</center>
...
`r msmbstyle::solution_begin(hidden = FALSE, toggle = TOGGLE)`
```{r message=FALSE}
acedata <- read_csv("../data/acedata.csv")
...
```{r}
# this is the probability of a t-statistic LESS THAN or equal to the abs(t)
pt(abs(t_stat), df = 24)

# So this is the probability of a t-statistic GREATER THAN or equal to the abs(t)
1 - pt(abs(t_stat), df = 24)

# And we will reject an t-statistic in either direction (in either tail), so we multiply by two.
...
#we need to know the critical value of t, for our desired alpha, with our df of 24
qt(0.95, df = 24)

# lower interval
aceterms$dbar - ( qt(0.975, df = 24) * (aceterms$s_d / sqrt(aceterms$n)))
# upper interval
aceterms$dbar + ( qt(0.975, df = 24) * (aceterms$s_d / sqrt(aceterms$n)))
```
...
We can reshape datasets using the functions `pivot_longer()` and `pivot_wider()`. 
...
Data on the ages of both brides and grooms at point of marriage is available [url](url).  
You can read it in using the code below: 

```{r echo=TRUE, message=FALSE}
marriages<-read_csv("../data/marriageages.csv")
```
...
3. Did the average number of cigarettes smoked per day differ between the groups at the inital recruitment?  
...
The data is available at [url](url).

```{r echo=FALSE}
cbtsmoke <- read.csv("../data/cbtsmoke.csv")
```
...
This is asking whether the mean number of cigarettes per day is different to 20. So we are asking whether a mean is different from an hypothesised number - this is a one sample $t$-test!  
...
To calculate Cohen's $D$, we use the mean difference ($\bar d$) divided by the standard deivation of the differences ($s_d$): 
