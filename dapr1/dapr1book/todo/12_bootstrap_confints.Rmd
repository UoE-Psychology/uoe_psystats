...
Last week, we learnt about how we can take a *statistic* from a sample to draw conclusions about a *parameter* in the population from which the sample is drawn. 
We then focused on how these sample statistics will vary from sample to sample. We saw that by taking lots of samples we could create a *sampling distribution* for a statistic, allowing us to quantify the variation in the sample statistics. Specifically, we learnt that the standard deviation of the sampling distribution is known as the *standard error*. 
Finally, we saw how the size of our samples influenced the sampling variation, with bigger samples leading to narrower sampling distributions, and more precise estimates.  
...
We mentioned last week that we often have neither the time nor the resources to collect data from the entire population (a *census*). It is also often infeasible to get *many samples* of size $n$ in order to get an idea of how accurate our estimate of the population parameter. 

**How can we study the variability of our sample statistic with only one sample??** 

It turns out, we can mimick the act of sampling $n$ units from the population, by *resampling with replacement* $n$ units from our original sample of $n$ units. This is broadly known as **bootstrapping**.
...
Think back to last week. We saw a dataset of **all** the National Football League players on 2015, along with their salaries. 
...
We can approximate the sampling distribution of $\bar{x}$ (the mean salary of our sample) by *bootstrap resampling with the replacement*. 
To do this, we:
...
If we resampled our 50 player sample *without* replacement, we would simply end up with the same 50 players!
...
Where do you think that this histogram is centered?
...
The distribution of means of resamples will be centered around the mean of our original sample!
```{r}
nfl_resample_means %>%
...
We have seen how the boostrapped distribution is an approximate of the sampling distribution. 

TRUE or FALSE: The standard deviation of the bootstrapped distribution is an approximation of the standard error of $\bar{x}$.
`r msmbstyle::question_end()`
`r msmbstyle::solution_begin(hidden=FALSE)`
TRUE!
`r msmbstyle::solution_end()`
...
+ **The green resample button**: Samples with replacement from the original sample, and calculates the mean (which is then dropped into the bottom panel)
+ **The bottom panel**: The distribution of resample means - the bootstrapped distribution!
...
The former, the confidence level, we set ourselves. For instance, we might ask between which values the middle 95% (or 90%, or 80%, etc.) of our distribution falls. 
...
We can construct confidence intervals using the **standard error** and the following rules of thumb:

<div class="think">
##### If a variable follows a normal curve... {-}
...
1. We resampled our original sample to create the bootstrap distribution (as an approximate to the sampling distribution).
1. We then used the bootstrap distribution to approximate the standard error of our sample statistic and construct a confidence interval to give us a range of plausible values for the population parameter. 
...
and we used the standard deviation of our bootstrapped resample means... 
```{r}
bootstrap_distribution %>%
...
giving us a confidence interval of $1.52$ to $2.7$.  
...
- *Confidence level.* The percentage of confidence intervals which will contain the true population parameter **in the long run** (i.e., if you sampled the population and constructed confidence intervals many times over). 
