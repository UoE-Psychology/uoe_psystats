...
We have many equivalent options to create a table of counts.

_Option 1. Using `tidyverse`:_
```{r}
writers %>% 
  group_by(DayOfWeek) %>% 
  summarise(count = n())
...
_Option 2. Using the function `xtabs` introduced in [Week 17](#two-indep-samples):_
```{r}
xtabs(~ DayOfWeek, data = writers)
```


_Option 3. Using the function `table`:_
```{r}
