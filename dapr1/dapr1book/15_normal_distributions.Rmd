...
- __The lab questions are important, as they are not just a copy of the walkthrough. They consolidated some key points covered in the lecture, but not the walkthrough.__
...
1. The total area under the graph of the equation must be equal to 1, equivalent to the fact that the total probability must be 1;
...
2. _Create a grid of values for the length of pregnancy variable. Recall from [Week 12](#calculating-confidence-intervals-using-a-bootstrap-standard-error) that 99.7% of all values lie between the mean plus or minus 3 standard deviations. To be sure to capture all of the values, use the mean plus or minus 4 standard deviations as limits._
