...
In order to find the probability of your subject losing attention between 10 and 20 minutes after the start of the experiment, we must calculate the shaded area in Figure \@ref(fig:shaded).
...
The normal density curve depends on two quantities:

- the mean of the distribution, $\mu$, specifying the location of the centre
- the standard deviation of the distribution, $\sigma$, specifying the spread of the distribution


**Varying the mean controls the location of centre of the distribution**:

```{r vary-means, fig.width=8, fig.height=4.5, fig.align='center', fig.cap='The effect of the mean on the location of the normal distribution.'}
library(tidyverse)

x <- seq(-4, 4, length.out = 1000)

df_mean_0 <- tibble(x = x, 
                    dens = dnorm(x, mean = 0, sd = 1), 
                    mean = 0)

df_mean_2 <- tibble(x, 
                    dens = dnorm(x, mean = 2, sd = 1), 
                    mean = 2)
...
ggplot(df_mean, aes(x = x, y = dens, color = as.factor(mean))) +
  geom_line(size = 1) +
  labs(title = 'Varying the mean', color = 'mean')
```

**Varying the standard deviation controls the narrowness or spread of the distribution**:

```{r vary-sds, fig.width=8, fig.height=4.5, fig.align='center', fig.cap='The effect of the mean on the location of the normal distribution.'}
x <- seq(-10, 10, length.out = 1000)

df_sd_3 <- tibble(x = x, 
                  dens = dnorm(x, mean = 0, sd = 3), 
                  sd = 3)
...
ggplot(df_sd, aes(x = x, y = dens, color = as.factor(sd))) +
  geom_line(size = 1) +
  labs(title = 'Varying the standard deviation', color = 'sd')
```

### NFL average yearly salary
...
We had data on 2,099 players, including their name, position, team they played for, total money while on a NFL payroll and the yearly salary.
...
Each bar in the histograms shows how many statistics fall in a given interval, such as the count of sample means between 2.2 and 2.3.

The distribution of the sample means seems to be symmetric and bell-shaped. Let's try adding the same red curve that we saw before, which is called the __normal curve__:
...
ggplot(sampling_distrib_of_mean) +
  geom_histogram(aes(x = avg, y = stat(density)), 
                 color = "white", boundary = 2, binwidth = 0.1) +
...
If we divide the counts by the total number of samples (10,000), we obtain the proportion of statistics in the interval considered, e.g. the proportion of sample means between 2.2 and 2.3.
