...
Each bar in the histogram shows the proportion of all the generated statistics falling in a given interval, such as the proportion of sample means between 2.2 and 2.3 which is equal to `r prop %>% round(2)`.
...
The probability of seeing an even face when throwing a die is obtained by dividing the number of ways the event "even face" can occurr (3 even faces in a die: ⚁, ⚃, ⚅) by the total number of possibilities (6 total faces in a die: ⚀, ⚁, ⚂, ⚃, ⚄, ⚅):
$$
...
# load the sample
cognitive <- read_csv('data/cognitive_sample.csv', col_names = TRUE)
...
However, on the y-axis, instead of the crude counts of the values, we ask for the density or proportion.
This makes it easier to compare across samples of different sizes, as the range of the y-axis won't be absolute counts and therefore change with the sample size.
...
Hence the normal curve should be centred at the mean of the data, `r sample_mean` and have standard deviation `r sample_sd`.


We specify a normal density curve with the following command:
```{r, eval=FALSE}
dnorm(x, mean, sd)
```
...
- `mean` the centre of the normal curve
...
We compute areas for normal curves using the function `pnorm(x, mean, sd)`. This function computes the area to the left of x in a normal curve centred at `mean` and having standard deviation `sd`.

**Area to the left of a value x**
```{r, echo=FALSE, out.width='80%', fig.align='center'}
knitr::include_graphics('images/pnorm_left.png')
```

**Area between the values $x_l$ and $x_u$**
...
The probability that a randomly selected individual has a cognitive score between 120 and 130 as the area under the normal curve between 120 and 130 is:
```{r}
...
Today we saw that when a histogram shows that a variable's distribution is symmetric and bell-shaped, we can say that the variable is normally distributed and we can model the distribution with a mathematical curve called the normal probability distribution.
...
The normal curve is given by the function `dnorm(x, mean, sd)`.

We can use the normal curve to compute probabilities. The function to compute the area to the left of `x` under a normal curve is `pnorm(x, mean, sd)`.


1. Probability of observing a value less than or equal `x`: `pnorm(x, mean, sd)`
2. Probability of observing a value between `xl` and `xu`: `pnorm(xu, mean, sd) - pnorm(xl, mean, sd)`
2. Probability of observing a value greater than `xu`: `1 - pnorm(xu, mean, sd)`
...
In today's lab we will fit a normal curve to the sampling distribution of the mean from the [NFL example](#sec-sampling-distributions) we saw in Week 11.
...
`r msmbstyle::solution_begin()`
The distribution of the sample means seems to be symmetric and bell-shaped, hence it is approximately normally distributed.
`r msmbstyle::solution_end()`


`r msmbstyle::question_begin(header = "&#x25BA; Question 3")`
Find the normal distribution that models the sampling distribution of the mean for samples of size $n = 100$.
...
`r msmbstyle::solution_begin()`
The distribution of sample means is centred at the population mean. In other words, the average of the sampling distribution of the mean is equal to the population mean.
`r msmbstyle::solution_end()`


`r msmbstyle::question_begin(header = "&#x25BA; Question 7")`
Do you notice any relation between the population standard deviation and the standard deviation of the sampling distribution of the mean?
...
`r msmbstyle::solution_begin()`
Assume that the population has mean $\mu$ and standard deviation $\sigma$.

It is possible to prove that the distribution of the mean, computed on samples of size $n$, is centre at the population mean, $\mu$ and has standard error $$SE(\bar{x}) = \frac{\sigma}{\sqrt{n}}$$.
...
_ _Simulation-based SE of the mean._ The standard deviation of the sampling distribution of the mean, obtained by generating many random samples and calculating the mean for each sample.
_ _Theory-based SE of the mean._ For a population with mean $\mu$ and standard deviation $\sigma$, the SE of the mean computed on samples of size $n$ is $SE(\bar{x}) = \frac{\sigma}{\sqrt{n}}$.
