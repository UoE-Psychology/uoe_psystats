...
`r msmbstyle::question_begin()`
Take a look again at the bootstrap distribution we constructed (Figure \@ref(fig:bootstrap_dist), below). Roughly, between what two values do most of the resample means lie?
```{r bootstrap_dist, echo=FALSE, fig.cap="Bootstrap resampling distribution based on 2000 resamples", message=FALSE}
ggplot(nfl_resample_means, aes(x = avg_salary)) + 
  geom_histogram(color = "white") + 
  labs(x = "resample mean")
```
...
which became:  
<br>
<center>$2.11 \pm 1.96 \times 0.3$</center>
<br>
