...
Probability p(x) **can't** be linear.  
...
We can, from these numbers, get out the predicted probability of survival using the inverse logit $p_i=\frac{e^L_i}{(1+e^L_i)}$ (this is the inverse of the log odds formula $L_i=log(\frac{p_i}{1-p_i}))$.  
```{r, }
...
`r subtask()`Use the `ifelse()` function to assign values of 0 and 1 to the `p_surv` predictions based on whether they are above or below 0.5.  
```{r, include=solution, class="solution"}
titanic$p_surv <- ifelse(titanic$p_surv > 0.5, 1, 0)
```
`r subtask()` Create a table of these against the `survived` variable (the observed data).  
```{r,include = solution, class="solution"}
table(
...
`r task()`How do age, sex, and ticket class predict the survival of passengers on the titanic?  
`r subtask()`How does a passenger being male (relative to female) influence the **odds** of survival?  
```{r}
better_model <- glm(survived ~ age + sex + pclass, data = titanic, family=binomial)

exp(coef(better_model))
# 0.08 odds of survival for males relative females

```
`r subtask()`**Extra:** Try doing a bit of googling to find the function which can quickly give you some confidence intervals for your predictors.  
```{r}
# remember to exp()!
exp(confint(better_model))
```
