...
```
`r task()`How does a passenger being male (relative to female) influence the **odds** of survival?  
```{r,include = solution, class.source="solution"}
exp(coef(titanic_model))
# 0.07 odds of survival for males relative females
```
`r task()`**Extra:** Try doing a bit of googling to find the function which can quickly give you some confidence intervals for your betas.  
```{r,include = solution, class.source="solution"}
# remember to exp()!
