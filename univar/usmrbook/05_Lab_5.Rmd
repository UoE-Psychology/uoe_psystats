...
```{r, eval=F, echo=solution, class.source="solution"}
...
`r task()`Get rid of the negative values by replacing them with `NA`s. **Hint. **again, logical subsetting is the way to go!
...
`r task()`Have a look at the distribution of `lab_time`. Use a histogram or a box plot to visualise it.
...
`r task()`Choose a safe cut-off point and use it to remove the offending data point.
...
`r task()`How can we programmatically spot non-integers?
...
`r task()`Use `%% 1` and logical subsetting to get rid of non-integer values.
...
```{r, fig.height=3.5, echo=solution, class.source="solution"}
barplot(lab_table)
```
...
```{r, echo = solution, include=solution, class.source="solution"}
...
Now that you have an inkling about how to use loops, let's see if we can make use of them to simulate our sampling distribution

`r task()`First, let's create an empty vector that will store each estimate of out *t*-statistic. Call it `t_est`, why not!

```{r, echo=solution, class.source="solution"}
t_est <- c()
```

`r task()`Next, let's write the core structure of a loop. Say we want the loop to run 10^4^ times. Don't worry about the code inside `{}` just yet.
...
`r task()`Now, inside of the loop, you want to sample the same number of observation as we have in our data from a normally distributed population with $\mu=103.7$ and *SD* of `sd(data$lab_time, na.rm = T)`. Save this sample into an object called `samp`.
...
`r task()`Now, we want to get the *t*-statistic for this sample and add it to our for now empty `t_est` vector. You can calculate it by hand if you wish but maybe you can somehow extract it from `t.test()`? **Hint: **`t.test()` returns a list of values. This list, just like any list, can be queried using `$`. The *Value* section of the function documentations will tell you the rest.
...
`r task()`That's it! You can run the loop now and sit back

`r task()`Let's look at the histogram of `t_est`. To see it in a higher resolution, let's ask for 50 breaks.
