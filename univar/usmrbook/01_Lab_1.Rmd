...
`r task()`Select <kbd>File</kbd> > <kbd>New File</kbd> > <kbd>R Script</kbd>. In the window that appears, click on the [floppy disk](https://en.wikipedia.org/wiki/Floppy_disk)[^1] icon (![](images/saveicon.png){width=25px}) just below *Untitled1* and save the (blank) script as `myscript.R`
...
```{r nor, fig.cap="**Fig. 3:** Distribution produced using `seq()`, `dnorm()`, and `plot()`.", echo=solution, class.source="solution"}
x <- seq(-3.5, 3.5, length.out = 49)
