...
`r subtask()`What does the next command do?

```{r eval = solution}
vec[8]
```

`r subtask()`What about the following command?
...
`r subtask()`What does the next command do (you have seen this before in previous labs - think about adding values to the empty matrix)?

```{r eval = solution}
mat[2, 3]
```

`r subtask()`What about the next one?

```{r eval = solution}
mat[ , 3]
```

`r subtask()`What's the difference between the folllowing two commands?
